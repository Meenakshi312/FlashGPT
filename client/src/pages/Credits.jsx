  ...
  return (
    <div className='max-w-7xl h-screen overflow-y-scroll mx-auto px-4 sm:px-6 lg:px-8 py-12'>
      ...
      <div className='flex flex-wrap justify-center gap-8'>
        {plans.map((plan)=>(
            <div key={plan._id} className={`border border-gray-200 dark:border-blue-700 rounded-lg shadow hover:shadow-lg transition-shadow duration-500 p-6 min-w-[300px] flex flex-col ${plan._id === "pro" ? "bg-blue-50 dark:bg-blue-900" : "bg-white dark:bg-transparent"}`}>
                <div className='flex-1'>
                    <h3 className='text-xl font-semibold text-gray-900 dark:text-white mb-2'>{plan.name}</h3>
                    <p className='text-2xl font-bold text-blue-600 dark:text-blue-300 mb-4'>€{plan.price}
                        <span className='text-base font-normal text-gray-600 dark:text-blue-200'>{' '}/ {plan.credits} credits</span>
                    </p>
                    <ul className='list-disc list-inside text-sm text-gray-700 dark:text-blue-200 space-y-1'>
